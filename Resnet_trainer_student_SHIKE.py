...
from loss import SHIKE_dkd_no_labels_loss
...
def get_parser():
    ...
    parser.add_argument("--t1_ckpt", type=str, default='/data4/tongshuo/Grading/CommonFeatureLearning/results/teacher1/resnet18_dr012_max_acc_ce.pth')
    parser.add_argument("--t2_ckpt", type=str, default='/data4/tongshuo/Grading/CommonFeatureLearning/results/teacher1/resnet50_dr012_max_acc_ce.pth')
    parser.add_argument("--t3_ckpt", type=str, default='/data4/tongshuo/Grading/CommonFeatureLearning/results/teacher2/resnet18_dr34_max_acc_ce.pth')
    parser.add_argument("--t4_ckpt", type=str, default='/data4/tongshuo/Grading/CommonFeatureLearning/results/teacher2/resnet50_dr34_max_acc_ce.pth')
    parser.add_argument("--dataset", type=str, default='dr',
                        choices=['dr', 'dr012' , 'dr34'])
    return parser
...
def amal(cur_epoch, criterion, criterion_ce, criterion_cf, model, cfl_blk, teachers, optim, train_loader, device, scheduler=None, print_interval=100):
    ...
    for cur_step, (images, labels) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            t_out_1 = torch.cat((t1_out, t3_out), dim=1)
            t_out_2 = torch.cat((t1_out, t4_out), dim=1)
            t_out_3 = torch.cat((t2_out, t3_out,), dim=1)
            t_out_4 = torch.cat((t2_out, t4_out,), dim=1)
            [t_outs.append(t) for t in [t_out_1, t_out_2, t_out_3, t_out_4]]

            # t_outs = F.softmax(t_outs, dim=1)

            ft1 = t1.layer4.output
            ft2 = t2.layer4.output
            ft3 = t3.layer4.output
            ft4 = t4.layer4.output
...
########将损失替换为SHIKE_DKD_loss#######
        loss_ce = SHIKE_dkd_no_labels_loss(s_outs, t_outs, alpha, beta, temperature, device)
...
def main():
    ...
    mkdir_if_missing('checkpoints')
    max_acc_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_max_acc_ce_SHIKE_1.pth'%(opts.model, opts.dataset)
    min_mae_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_min_mae_ce_SHIKE_1.pth'%(opts.model, opts.dataset)
    #  Set up dataloader
    ...
    t2 = _model_dict[t2_model_name](num_classes=3).to(device)
    t3_model_name = 'resnet18'
    t3 = _model_dict[t3_model_name](num_classes=2).to(device)
    t4_model_name = 'resnet50'
    t4 = _model_dict[t4_model_name](num_classes=2).to(device)
    ...
    print("Loading pretrained teachers ...\nT1: %s, T2: %s, T3: %s, T4: %s"%(t1_model_name, t2_model_name, t3_model_name, t4_model_name))
    t1.load_state_dict(torch.load(opts.t1_ckpt)['model_state'])
    t2.load_state_dict(torch.load(opts.t2_ckpt)['model_state'])
    t3.load_state_dict(torch.load(opts.t3_ckpt)['model_state'])
    t4.load_state_dict(torch.load(opts.t4_ckpt)['model_state'])
    t1.eval()
    t2.eval()
    t3.eval()
    t4.eval()
    ...
    t3_feature_dim = t3.fc.in_features #512
    t4_feature_dim = t4.fc.in_features #512
    ...
    cfl_blk = CFL_ConvBlock(stu_feature_dim, [t1_feature_dim, t2_feature_dim, t3_feature_dim, t4_feature_dim], 128).to(device)
    ...
    t3.layer4.register_forward_hook(forward_hook)
    t4.layer4.register_forward_hook(forward_hook)
    ...
    while cur_epoch < opts.epochs:
        ...
        score_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_score_student_ce_SHIKE_1.txt'%(opts.model)
        score_best_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_score_student_best_ce_SHIKE_1.txt'%(opts.model)
