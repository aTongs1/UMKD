...
#####它会创建多个残差块（block），并且如果需要，添加下采样（downsample）操作
    ...
    #     return x
    def forward(self, x, uncertainty_type='max_modified_prob', lamb1=1.0, lamb2=0.1, score_norm=False):
...
######### Calculate uncertainty 
        ...
        if score_norm:
            ...
            # return x, 1 / np.exp(scores_norm.mean())
            return x, (math.e - np.exp(scores_norm.mean())) / (math.e + 1)
        else:
            # return x, 1 / np.exp(scores.mean())
            return x, (math.e - np.exp(scores.mean())) / (math.e + 1)
            # 计算公式 (e - e^(x)) / (e + 1)


def resnet18(pretrained=False, num_classes=None, **kwargs):
...
def resnet34(pretrained=False, num_classes=None, **kwargs):
...
def resnet50(pretrained=False, num_classes=None, **kwargs):
...
def resnet101(pretrained=False, num_classes=None, **kwargs):
...
def resnet152(pretrained=False, num_classes=None, **kwargs):
...
# # 示例输入
# input_data = torch.randn(18, 3, 224, 224)  # 一张224x224的RGB图像
...
# # 进行多次前向传播并获取均值和方差
# output, uncertainty = model(input_data, uncertainty_type='max_modified_prob', lamb1=1.0, lamb2=1.0, score_norm=False)

# # 输出均值和方差
# print(f'Mean Output Shape: {output.shape, output}')
