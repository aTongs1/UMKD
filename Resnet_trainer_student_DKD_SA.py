...
from loss import SoftCELoss, CFLoss, CFLoss_SA
...
def amal(cur_epoch, criterion, criterion_ce, criterion_cf, criterion_cf_SA, model, cfl_blk, cfl_blk_SA, teachers, optim, train_loader, device, scheduler=None, print_interval=100):
...
###############################################################################  
        ft_SA = [ft1_SA, ft2_SA]
        (hs_SA, ht_SA), (ft_SA_, ft_SA) = cfl_blk_SA(fs_SA, ft_SA)
###############################################################################
        ft = [ft1, ft2]
...
########将损失替换为SHIKE_DKD_loss#######
        ...
        loss_cf_SA = 10*criterion_cf_SA(hs_SA, ht_SA) #浅层特征的MMD损失,没有重构损失

        # loss = loss_ce + loss_cf
        # loss = loss_1 + loss_ce + loss_cf
        loss = loss_1 + loss_ce + loss_cf + loss_cf_SA
        loss.backward()
        optim.step()
...
def main():
    ...
    mkdir_if_missing('checkpoints')
    max_acc_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_max_acc_ce_DKD_50_50_01234_notran_SA.pth'%(opts.model, opts.dataset)
    min_mae_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_min_mae_ce_DKD_50_50_01234_notran_SA.pth'%(opts.model, opts.dataset)
    #  Set up dataloader
    ...
    stu = _model_dict[opts.model](pretrained=True, num_classes=num_classes).to(device)
...
##### Loss #####
    ...
    criterion_cf_SA = CFLoss_SA(normalized=True)
    ...
    while cur_epoch < opts.epochs:
        ...
        amal(cur_epoch=cur_epoch,
                            ...
                            criterion_cf_SA=criterion_cf_SA,
                            model=stu,
                            cfl_blk=cfl_blk,
        ...
        score_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_score_student_ce_DKD_50_50_01234_notran_SA.txt'%(opts.model, opts.dataset)
        score_best_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_score_student_best_ce_DKD_50_50_01234_notran_SA.txt'%(opts.model, opts.dataset)
