...
from models.PVT_Linear import PVT_Linear
...
def get_parser():
    ...
    parser.add_argument("--dataset", type=str, default='dr01234',
                        choices=['dogs', 'cub200', 'dr012', 'dr34', 'dr01234'])
    parser.add_argument("--model", type=str, default='pvt_linear')
...
def train_one_epoch(cur_epoch, criterion, model, optim, train_loader, device, scheduler=None, print_interval=50):
    ...
    for cur_step, (images, labels) in enumerate(train_loader):
        ...
        # labels_onehot = torch.zeros((labels.size(0), 5), device=device)
...
def main():
    ...
    if opts.dataset == 'cub200':
        ...
        dataset = DRDataset
        num_classes = 5
    pvtlinear = _model_dict[opts.model]
    #latest_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/checkpoints/%s_%s_latest_es.pth'%(opts.dataset, opts.model)
    max_acc_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_%s_max_acc_ce.pth'%(opts.dataset, opts.model)
    min_mae_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_%s_min_mae_ce.pth'%(opts.dataset, opts.model)
    # Set up dataloader
    '''
    ...
    model = pvtlinear(num_classes=num_classes).to(device)
...
#######损失函数替换为针对类别不平衡的BCE Loss
    ...
    while cur_epoch < opts.epochs:
        ...
        score_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_score_dr01234_ce.txt'%(opts.model)
        score_best_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_score_dr01234_best_ce.txt'%(opts.model)
