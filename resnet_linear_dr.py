...
def main():
    ...
    max_acc_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_%s_max_acc_ce_notran_nopt.pth'%(opts.model, opts.dataset)
    min_mae_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_%s_min_mae_ce_notran_nopt.pth'%(opts.model, opts.dataset)
    # Set up dataloader
    '''
...
#####尝试一下不添加预训练的权重#####
    model = resnet(pretrained=False, num_classes=num_classes).to(device)
...
#######损失函数替换为针对类别不平衡的BCE Loss
    ...
    while cur_epoch < opts.epochs:
        ...
        model.eval()           
        val_score, val_f1, val_recall, val_precision, measure_result = validate(model=model,
                             loader=val_loader,
                             device=device,
                             metrics=metrics)
        print("Confusion Matrix:\n",val_score['Confusion Matrix'])
        print(metrics.to_str(val_score))
        score_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_%s_score_ce_notran_nopt.txt'%(opts.model, opts.dataset)
        score_best_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/baselines/%s_%s_score_best_ce_notran_nopt.txt'%(opts.model, opts.dataset)
