...
from models.resnet_LowPass import resnet18_LP, resnet34_LP, resnet50_LP
...
################把之前的MMD损失替换为欧氏距离##################      
        ...
        if (cur_step+1) % print_interval == 0:
            ...
            print("Epoch %d, Batch %d/%d, Loss=%f (cls=%f, tckd=%f, nckds=%f, cf=%f, cf_LP=%f), Acc=%f, MAE=%f"  %
                  (cur_epoch, cur_step+1, len(train_loader), interval_loss, loss_1, loss_tckd, loss_nckds, loss_cf, loss_cf_LP, acc, mae))
...
def main():
    ...
    mkdir_if_missing('checkpoints')
    max_acc_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/LP/%s_%s_%s_max_acc_ce_DKD_50_50_18_notran_LP_euc.pth'%(opts.model, opts.dataset, opts.layer)
    min_mae_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/LP/%s_%s_%s_min_mae_ce_DKD_50_50_18_notran_LP_euc.pth'%(opts.model, opts.dataset, opts.layer)
    #  Set up dataloader
...
##### Loss #####
    ...
    while cur_epoch < opts.epochs:
        ...
        score_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/LP/%s_%s_%s_score_student_ce_DKD_50_50_18_notran_LP_euc.txt'%(opts.model, opts.dataset, opts.layer)
        score_best_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/LP/%s_%s_%s_score_student_best_ce_DKD_50_50_18_notran_LP_euc.txt'%(opts.model, opts.dataset, opts.layer)
