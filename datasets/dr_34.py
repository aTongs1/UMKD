...
class DRDataset34(data_utils.Dataset):
    ...
    def __init__(self, patch_level, img_root, dataset, transform=None, fold=3):
        ...
        # 提取第二个元素为3或4的子列表
        self.data_list = [item for item in self.data_list if item[1] in {'3', '4'}]
        # 对每个子列表的第二个元素进行减3操作
        for item in self.data_list:
            item[1] = str(int(item[1]) - 3)  # 将第二个元素转为整数并减去1，再转回字符串
...
if __name__ == '__main__':
    ...
    # print(tran)
    train_data = DRDataset34(None,None,'train',tran,0)
