...
from models.resnet_SDD import resnet18_sdd, resnet34_sdd, resnet50_sdd
...
_model_dict = {
    'resnet18': resnet18_sdd,
    'resnet34': resnet34_sdd,
...
def amal(cur_epoch, criterion, criterion_ce, criterion_cf, model, cfl_blk, teachers, optim, train_loader, device, scheduler=None, print_interval=2):
    ...
    for cur_step, (images, labels) in enumerate(train_loader):
        ...
        optim.zero_grad()
        with torch.no_grad():
            t1_out, patch_score_t1 = t1(images)
            t2_out, patch_score_t2 = t2(images)
        ...
        # get student output
        s_outs, patch_score_s = model(images)
        if is_densenet:
...
def main():
    ...
    mkdir_if_missing('checkpoints')
    max_acc_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_max_acc_ce_SDD_50_50_18_notran_nopt.pth'%(opts.model, opts.dataset)
    min_mae_ckpt = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_min_mae_ce_SDD_50_50_18_notran_nopt.pth'%(opts.model, opts.dataset)
    #  Set up dataloader
    ...
    stu = _model_dict[opts.model](pretrained=False, num_classes=num_classes, M = '[1,2,4]').to(device)
    ...
    while cur_epoch < opts.epochs:
        ...
        score_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_score_student_ce_SDD_50_50_18_notran_nopt.txt'%(opts.model, opts.dataset)
        score_best_path = '/data4/tongshuo/Grading/CommonFeatureLearning/results/student/%s_%s_score_student_best_ce_SDD_50_50_18_notran_nopt.txt'%(opts.model, opts.dataset)
